# MCP_KEYCLOCK_READ_ONLY Support Specification
...
- [ ] Add `MCP_KEYCLOCK_READ_ONLY` env var (boolean, default `false`).
...
`MCP_KEYCLOCK_READ_ONLY=true` so that only GET (read) Keycloak operations are exposed as
...
1. WHEN `MCP_KEYCLOCK_READ_ONLY` is unset or `false` THEN system SHALL register all
   operations as tools (current behavior unchanged).
2. WHEN `MCP_KEYCLOCK_READ_ONLY=true` THEN system SHALL register only operations whose
   HTTP method is `GET` as MCP tools.
3. WHEN `MCP_KEYCLOCK_READ_ONLY=true` AND an operation's method is not `GET` THEN system
...
**Independent Test**: Set `MCP_KEYCLOCK_READ_ONLY=true`, run server, list tools via MCP
...
| ---- | -------------------------------------------------------------------------------------- | -------- |
| FR-1 | `Settings.read_only: bool` field, env `MCP_KEYCLOCK_READ_ONLY`, default `False`        | P1 AC1-2 |
...
| FR-5 | README env var table documents `MCP_KEYCLOCK_READ_ONLY`                                | Docs     |
